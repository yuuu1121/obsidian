...
> [!note] 이 목록의 성격
> 저자들의 **집단적 과거·진행 중 연구에서 도출된 큐레이션**이며, 스스로 *"완전하지는 않다(not exhaustive)"* 고 밝힌다. 다만 **실무 구현과 이론적 진전 양쪽**을 겨냥한 견고한 출발점이라고 자평한다.
>
> 아래 8가지를 관통하는 구조가 있다. **①③⑧은 리프팅이 만든 "고차원 선형"이라는 성질의 부작용**이고, **④⑤⑥은 특정 시스템 클래스**의 문제, **②⑦은 확률·불확실성**의 문제다.
...
로봇 제어는 항상 제약과 함께 온다 — 관절 각도 한계, 토크 한계, 장애물 회피, 안전 영역. 이들은 **원 상태공간 $\mathcal{X}$ 에서 자연스럽게 정의**된다. 예: $\|x - x_{\text{obs}}\| \ge r$.

그런데 [[Koopman MPC]]는 **리프팅 공간에서** 최적화를 푼다. 그러면 제약도 $z = \psi(x)$ 의 언어로 번역해야 한다.

> [!warning] 왜 번역이 어려운가
> 리프팅은 $\mathcal{X}\to\mathbb{R}^{N_\psi}$ 로 가는 **비선형** 사상이다. 원 공간의 단순한 제약이 리프팅 공간에서는 **복잡한 비볼록 집합**이 될 수 있다.
>
> 예를 들어 원 공간의 볼록한 원 $\|x\|\le r$ 도, $\psi$ 에 $x^2$ 나 $\sin x$ 가 섞여 있으면 리프팅 공간에서 휘어진 곡면 위의 영역이 된다. 그러면 §7-B에서 애써 얻은 **볼록성**이 제약 때문에 깨진다.
>
> 반대로 리프팅 공간에서 볼록한 제약을 걸면 계산은 쉽지만, 그것이 **원 공간에서 무엇을 의미하는지** 알기 어렵다. 안전 제약이라면 이건 치명적이다.

**논문의 입장**

*"우리가 일부 방법을 소개했지만, 원 공간의 다양한 제약을 Koopman 공간으로 어떻게 리프팅할지는 더 많은 연구가 필요하다."* 그리고 열쇠로 **Koopman 연산자의 대수적·기하학적 구조 활용**을 지목한다 — 예컨대 불변 부분공간의 대수적 성질([[Koopman-Invariant Subspace]])이 제약 집합의 구조를 보존하는 조건을 줄 수 있다는 기대다.

---

### ② 확률적 시뮬레이션과 belief-space 계획

**무엇이 문제인가**

지금까지의 논의는 대부분 **결정론적**이었다. 그런데 실제 로봇은 센서 노이즈, 모델 오차, 환경 불확실성 속에서 동작한다. 그러면 상태를 하나의 점이 아니라 **분포(belief)** 로 다뤄야 하고, 계획도 그 분포 위에서 해야 한다 — 이것이 **belief-space planning**이다.

> [!warning] 다봉(multimodal) 분포가 특히 어렵다
> "로봇이 문 왼쪽으로 갔을 수도, 오른쪽으로 갔을 수도 있다" 같은 상황에서 belief는 봉우리가 둘인 분포가 된다. 가우시안 하나로는 표현할 수 없고, 평균을 취하면 **어느 쪽도 아닌 문 한가운데**라는 무의미한 값이 나온다.
>
> 이런 분포를 리프팅 공간에서 전파하고, 그 위에서 robust한 계획을 세우는 것이 과제다.

**시도된 것 / 남은 것**

**확률적 Koopman 연산자(stochastic Koopman operator)** [65][164][165]가 이 방향의 진전이다. 확률적 동역학에 대해 관측값의 **기댓값**을 전파하는 연산자를 정의한다.

$$
[\mathcal{K}g](x) = \mathbb{E}\big[g(x^+)\,\big|\,x\big]
$$

다만 논문은 *"효율적 구현, 신뢰성, 적응성을 연구할 필요가 더 있다"* 고 못박는다. 이론적 정의는 있으나 **로봇에서 실시간으로 돌릴 만큼 성숙하지 않았다**는 뜻이다.
...
Koopman을 이산시간으로 쓰면 $\Delta t$ 를 정해야 한다. 이건 단순한 구현 세부사항이 아니다.

> [!important] $\Delta t$ 가 결정하는 세 가지
> **1. 학습되는 연산자 자체가 달라진다.** §7-A에서 봤듯 $\mathcal{K}^{\Delta t} = e^{\Delta t\,\mathcal{L}_G}$ 이므로, $\Delta t$ 를 바꾸면 **다른 연산자**를 학습하는 것이다.
>
> **2. 예측 오차 bound를 좌우한다.** 논문이 명시적으로 *"it governs the error bounds associated with predictions"* 라고 쓴다.
>
> **3. 온라인/적응 학습에서 특히 치명적이다.** 모델 갱신이 데이터의 시간 해상도에 크게 의존하기 때문이다.

**증거**

최근 연구 [166]가 **불충분한 샘플링이 외란 추정과 제어 정확도를 저하시킨다**는 것을 보였다. 즉 이건 이론적 우려가 아니라 **측정된 현상**이다.

> [!tip] 예제로 직접 확인할 수 있다
> [[Koopman 예제 코드]]의 03번 스크립트에 이 문제가 재현되어 있다. 원본 튜토리얼은 데이터를 $\Delta t = 0.05$ 로 수집한 뒤 MPC에서 $\Delta t = 0.1$ 로 덮어쓰는데, 학습된 모델의 시간 스케일과 시뮬레이션이 어긋나 성능이 떨어진다.

**남은 것**: "이 시스템에는 이 $\Delta t$" 를 **원리적으로 고르는 방법**이 없다. 현재는 시행착오다.

---

### ④ 정교·민첩(dexterous) 조작으로의 확장

**왜 유망한가**

접촉이 풍부한(contact-rich) 조작 — 미세한 물체 상호작용, **비파지(nonprehensile) 제어**(밀기·굴리기처럼 쥐지 않고 다루기), 손가락 조작 — 이 로보틱스의 중심 과제가 되고 있다. Koopman이 여기서 강점을 가질 이유가 둘 있다.

| 강점 | 내용 |
|:---|:---|
| **불연속성 처리** | 접촉은 붙었다 떨어지는 **불연속** 동역학을 만든다. 전역 선형 표현이 이것을 다룰 잠재력이 있다 (→ ⑥ 하이브리드와 연결) |
| **데이터 기반** | 물체마다 미묘하게 다른 거동은 **해석적으로 모델링하기 어렵다**. 데이터로 잡으면서도 최적화하기 좋은 구조를 유지할 수 있다 |

**현황**: Han et al. [41]의 손-물체 결합 리프팅(§IV-A) 같은 초기 사례가 나왔지만, 논문은 *"이 방향은 아직 초기 단계(nascent)"* 라고 평가한다.

---

### ⑤ 소프트 로보틱스 심화 — 리프팅의 근본적 한계

> [!danger] 이 항목이 가장 아프다 — Koopman의 핵심 메커니즘이 곧 한계가 된다
> Koopman의 전략은 **차원을 올려 선형화하는 것**이다. 그런데 소프트 로봇은 **원래부터 자유도가 사실상 무한**하다(연속체이므로).
>
> 이미 고차원인 것을 **더 높은 차원으로 올리면** 계산 비용이 과도해지거나 아예 **다루기 불가능(intractable)** 해진다. §IV-C에서 소프트 로봇을 "Koopman의 킬러 앱"이라 했던 것과 정면으로 충돌하는 지점이다.

**딜레마**

효과적 선형화를 위해 **차원 축소**나 **단순화된 파라메트릭 표현**이 필요해진다. 그런데 이 근사들이

- 불가피하게 **편향(bias)** 을 도입하고
- **일반화 능력을 제한**한다
- 특히 **접촉이 많거나 큰 변형**이 일어나는 작업에서 심각하다

**남은 질문**: *"Koopman 방법과 호환되는 소프트 로봇의 올바른 기술(description)을 어떻게 고를 것인가."* 즉 문제는 알고리즘이 아니라 **상태를 무엇으로 잡을 것인가**라는 더 근본적인 지점에 있다.
...
논문은 §II(이산시간)와 §V-A(연속시간)를 **각각 독립적으로** 다뤘다. 그런데 실제 메카트로닉스·로봇 시스템의 상당수는 **하이브리드**다 — 연속적으로 흐르다가 특정 순간 이산적으로 점프한다.

| 예 | 연속 부분 | 이산 점프 |
|:---|:---|:---|
| 다리 로봇 | 공중 궤적 | **접촉 순간** 속도 불연속 |
| 조작 | 물체 이동 | **잡기/놓기** 전환 |
| 재활 로봇 | 관절 운동 | **접촉/비접촉** 모드 전환 |

> [!warning] 왜 미묘한가
> Koopman 연산자는 $\mathcal{K}g = g\circ T$ — **$T$ 와의 합성**으로 정의된다. 하이브리드 시스템은 $T$ 가 **어느 모드에 있느냐에 따라 달라지고**, 모드 전환 자체가 상태에 의존한다. 그러면 "하나의 $\mathcal{K}$" 를 정의하는 것부터 자명하지 않다.

**진전과 한계**

Asada [16]가 **특정 조건 하에서 unforced 이종(heterogeneous) 시스템**(하이브리드의 한 부류)을 고차원 선형 시스템으로 리프팅할 수 있음을 보였다. 그러나 논문은 남은 것을 명확히 한다.

- **입력이 있는 경우**로의 확장 → 미해결
- **더 넓은 하이브리드 동역학**이 지배하는 시스템 → 미해결

---

### ⑦ 리프팅된 특징의 불확실성 — 가우시안 가정은 정당한가

**무엇이 문제인가**

[[Fisher Information]]과 [[Koopman MPC]] 5번에서 봤듯, 능동학습과 상태 추정의 상당 부분이 **"근사 Koopman 연산자의 불확실성은 평균 0인 가우시안"** 이라는 가정 위에 서 있다. 논문은 이 가정을 **재검토해야 한다**고 말한다.

> [!important] 저자들이 던지는 정확한 질문
> *"상태 $x_t$ 가 가우시안으로 잘 모델링되는 시스템이 있다고 하자. 이 경우 기저 동역학도 선형일 가능성이 높다. 그런데 **가우시안 분포를 관측 공간(리프팅 공간)으로 밀어넣으면 여전히 가우시안이 되는가?** 그리고 어떤 조건에서 그런가?"*
>
> **답은 일반적으로 "아니오"** 다. 가우시안 확률변수 $x$ 에 비선형 함수 $\psi$ 를 적용하면 $\psi(x)$ 는 더 이상 가우시안이 아니다. 예를 들어 $x\sim\mathcal{N}(0,1)$ 이면 $x^2$ 는 카이제곱 분포다.
>
> 게다가 **제어 입력까지 더해지면** 관측값 불확실성의 성질은 더 불투명해진다.

**현재 상태**

[82]와 [167]은 이 특정 불확실성 모델에 **robust하게** 동작하는 제어기를 설계할 수 있음을 보였다 — 즉 가정이 틀려도 버티는 방법이다. 하지만 그건 우회이지 해결이 아니다.

**핵심 연구 방향**: *"리프팅 관측함수가 어떤 통계적 구조를 보존하는 경우를 찾는 것."* 만약 "이런 딕셔너리는 가우시안성을 보존한다"는 클래스를 특정할 수 있다면, Koopman + Kalman filter 계열([98][115][85])이 비로소 **이론적 정당성**을 얻는다.
...
리프팅 공간의 시스템은 (대개) 선형 또는 쌍선형 모델로 기술된다. 그리고 **고차원 Koopman 공간의 관측값을 (쌍)선형 모델로 표현하는 것은, 원래의 (저차원) 동역학이 비선형인 것과 모순되지 않는다** — 이것이 Koopman의 전체 요지다.

이 성질이 **runtime 적응**을 쉽게 만든다. 그런데 **제어를 위한 최적화 문제를 푸는 데는 오히려 난점**을 만든다.

> [!warning] 비용함수가 리프팅 상태에 의존하면 생기는 일
> 특수한 경우가 아니면 비용함수는 **리프팅 상태 $z$ 에 의존**하게 된다. 그러면 MPC 이론의 표준 도구들이 흔들린다.
>
> 대표적으로 **recursive feasibility**(재귀적 실행가능성) — *"지금 풀 수 있으면 다음 스텝에서도 풀 수 있는가"* 를 보장하는 성질이다. 이것이 없으면 MPC가 도중에 해를 못 찾고 멈출 수 있다.
>
> 그런데 recursive feasibility 증명은 보통 **비용함수와 종단 제약의 관계**를 이용하는데, 비용이 $z$ 의 함수이고 $z$ 가 원 상태의 비선형 사상이면 그 관계를 따지기가 훨씬 까다로워진다. 논문이 **Koopman-NMPC의 recursive feasibility 평가**를 콕 집어 언급하는 이유다.

**논문의 결론**: 신중한 구현으로 일부 완화할 수 있지만, *"로봇 시스템 전반에 일반화되는 적절한 비용함수를 어떻게 설계할 것인가는 열려 있다."*

---

> [!abstract] 8가지를 관통하는 하나의 주제
> 대부분의 항목이 결국 같은 곳을 가리킨다 — **"리프팅으로 얻은 고차원 선형성이 공짜가 아니다."**
>
> | 얻은 것 | 그 대가로 생긴 열린 문제 |
> |:---|:---|
> | 선형 동역학 | 제약(①)·비용함수(⑧)를 리프팅 공간으로 번역해야 함 |
> | 고차원 표현 | 이미 고차원인 시스템(⑤)에서 계산 폭발 |
> | 결정론적 선형대수 | 확률 구조(②⑦)를 어떻게 얹을지 불명확 |
> | 이산 또는 연속 정식화 | 둘이 섞인 하이브리드(⑥)는 미개척 |
>
> §2에서 짚은 *"선형성은 공짜가 아니다"* 라는 명제가, §VI에서 **8가지 구체적 청구서**로 돌아온 셈이다.
