...
chapter: 9
tags: [MPC, 모델예측제어, 명시적MPC, 다중파라메트릭계획법, 조각별아핀, 고속구현]
---
...
> [!warning] 원문 수식 확인 필요
> 추출 텍스트에서 식 (9.3)의 두 번째 항은 `|{\bf u}|^\mathrm{T}` 로 끊겨 있어 제어 신호에 곱해지는 가중 벡터의 이름이 명확하지 않다. 다만 이어지는 식 (9.6)의 행렬 $\mathbf{S}$에서 그 자리에 $\mathbf{c}^\mathrm{T}$가 등장하므로, 여기서는 $|\mathbf{u}|^\mathrm{T}\mathbf{c}$로 복원했다. (컨트롤러가 PDF로 확인 요망.)
...
$$J = \frac{1}{2}\left(a x(t) + b u(t)\right)^2 \cdot \frac{2}{2} + \cdots$$
...
> [!warning] 원문 수식 확인 필요
> 식 (9.14)와 $\mathbf{K}_a$ 식에서 괄호 안 두 번째 항의 부호가 추출 텍스트에서 서로 어긋난다 — (9.14)에서는 $-\mathbf{R}_a^\mathrm{T}(\cdots)$, $\mathbf{K}_a$에서는 $+\mathbf{R}_a^\mathrm{T}(\cdots)$로 되어 있다. 두 식은 같은 전개에서 나와야 하므로 부호가 일치해야 한다. 위에서는 추출 텍스트를 그대로 옮겼으니 **PDF로 부호 확인이 필요하다.** 개념적 결론(제어법칙이 파라미터의 아핀 함수)에는 영향이 없다.
...
>
> 약 **41배** 빠르다. 예제 9.1보다 배율이 줄어든 것은 결정 변수가 1개에서 5개로 늘어 영역 판정 비용이 커졌기 때문이다.
...
$$A = \begin{bmatrix}1 & 1\\ 0 & 1\end{bmatrix}, \qquad B = \begin{bmatrix}1\\ 1\end{bmatrix}, \qquad C = \begin{bmatrix}1 & 0\end{bmatrix}$$

> [!warning] 원문 수식 확인 필요
> 본문 서술의 $B = [1\;\;1]^\mathrm{T}$와, 같은 예제를 MPT3로 재현하는 Code 9.1의 `B = [0; 1]`이 서로 다르다. 이중 적분기를 영차 홀드로 이산화하면 보통 $B=[T^2/2\;\;T]^\mathrm{T} = [0.5\;\;1]^\mathrm{T}$가 나오므로 둘 다 그대로 받아들이기 어렵다. **PDF로 확인이 필요하다.** 아래 표 9.1의 수치는 원문 그대로 옮겼다.
...
> [!tip] 비유 — 스무고개
> "동물인가요?"로 후보의 절반을 날리는 질문이 좋은 질문이다. "혹시 코끼리인가요?"처럼 후보 하나만 걸러 내는 질문은 나쁜 질문이다. 좋은 질문만 고르면 $N$개 후보를 $\log_2 N$번 만에 찾아낸다. 영역 판정을 **선형 탐색($O(N)$)에서 이진 탐색($O(\log N)$)으로** 바꾸는 것이 이 아이디어의 전부다.
...
> [!warning] 원문 수식 확인 필요
> $D = [1\;\;0]$은 행벡터로 적혀 있으나, $D\theta(t)$가 2차원 상태에 더해지려면 **열벡터 $[1\;\;0]^\mathrm{T}$** 여야 차원이 맞는다. 전치 표기가 추출 과정에서 누락된 것으로 보인다. (PDF 확인 요망.)
...
**다음 챕터 예고.** 이 챕터로 우리는 "MPC를 어떻게 빠르게 실행할 것인가"라는 구현 측면의 질문에 답했다. 이어지는 챕터에서는 제어기 하나가 아니라 **여러 제어기가 협력하는 상황**, 그리고 실제 산업 응용에서의 MPC 운용으로 시야를 넓힌다.
