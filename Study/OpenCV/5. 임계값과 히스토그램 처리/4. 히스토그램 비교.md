...
- 두 개의 히스토그램 H1, H2를 method 방법으로 비교.
	- 1. method = cv2.HISTCMP_CORREL
		- ![[Attachments/4e761fc72034c22b958180792376a73a_MD5.jpeg|400]]
	...
	- 2. method = cv2.HISTCMP_CHISQR
		- ![[Attachments/880f99d2540d42f05faddf822a4cb3a0_MD5.jpeg|400]]
		- 값이 작을수록(0에 가까울수록) 두 이미지가 유사한 색상 분포를 가짐.
	- 3. method = cv2.HISTCMP_INTERSECT
		- ![[Attachments/2d5975a791da1f3cc9ecf31cba10428f_MD5.jpeg|400]]
...
```
![[Attachments/00283ac85819cb1cee6f683159e60689_MD5.jpeg|400]]
...
```
![[Attachments/ddfeaa02cf05c336b8c9f73e34a54ae2_MD5.jpeg|400]]
- `S1`과 `S2`는 `np.zeros((H1.shape[0], 2), dtype=np.float32)`로 생성된 **2D 배열.**
	- **`(256, 2)` 크기의 배열**이며, 2개의 열(column)을 가짐.
	- ![[Attachments/426822c40219f32b8315f4b066e4e73b_MD5.jpeg]]
	- `S1[i, 0] = H1[i, 0]`  
